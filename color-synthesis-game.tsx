...
const ColorSynthesis = () => {
  ...
  const calculateColorAtPoint = () => {
    if (isSubtractive) {
      // Mode soustractif - utiliser le même modèle que le mode de fusion multiply
      let r = 255, g = 255, b = 255; // On part du blanc
      ...
      circles.forEach(circle => {
        ...
        if (distance <= radius) {
          // Récupérer la couleur RGB du cercle
          const circleColor = getCircleColorRGB(circle);
          
          // Appliquer le mode multiply: multiply(a,b) = (a * b) / 255
          // Avec opacité de 0.85
          const opacity = 0.85;
  ...
  const getCircleColorRGB = (circle) => {
    if (isSubtractive) {
      ...
      if (circle.id === 'red') return { r: 255 - amount, g: 255, b: 255 }; // Cyan
      if (circle.id === 'green') return { r: 255, g: 255 - amount, b: 255 }; // Magenta
      if (circle.id === 'blue') return { r: 255, g: 255, b: 255 - amount }; // Jaune
    } else {
  ...
  const getCircleColor = (circle) => {
    if (isSubtractive) {
      // Couleurs soustractives CMY - moins de pigment = plus blanc
      const amount = circle.intensity;
      if (circle.id === 'red') return `rgb(${255 - amount}, 255, 255)`; // Cyan (absorbe rouge)
      if (circle.id === 'green') return `rgb(255, ${255 - amount}, 255)`; // Magenta (absorbe vert)
      if (circle.id === 'blue') return `rgb(255, 255, ${255 - amount})`; // Jaune (absorbe bleu)
    } else {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4" style={{ backgroundColor: isSubtractive ? '#ffffff' : '#000000' }}>
      <div className="mb-6 text-center">
        ...
        <p className="text-sm mt-2" style={{ color: isSubtractive ? '#6b7280' : '#9ca3af' }}>
          {isSubtractive 
            ? 'Cyan + Magenta = Bleu | Cyan + Jaune = Vert | Magenta + Jaune = Rouge'
            : 'Rouge + Vert = Jaune | Bleu + Rouge = Magenta | Bleu + Vert = Cyan'
          }
        </p>
      </div>
